...
/* Asked for alongside the enrolment certificate, because both go in the same
 * reply and asking twice is asking once too often.
 *
 * A photo is genuinely useful here: Dutch landlords letting a room in a shared
 * house are choosing a housemate, and a face attached to the letter does more
 * than another line about income. It is also plainly optional, and worded so
 * that somebody who would rather not is not made to feel awkward. */
const REQUEST_PHOTO = {
  title: 'A photo of yourself, if you are comfortable',
  body:
    'Optional, and nobody will think less of you for skipping it. But landlords letting a room ' +
    'in a shared house are choosing a housemate, not a tenant, and a friendly photo attached to ' +
    'your letter does more than another paragraph about your salary. One clear picture of you is ' +
    'plenty. If you are moving with a partner, one of you both.',
};
...
export function applicantEmail({ reference, application, receiptUrl, siteUrl, supportEmail, opening }) {
  ...
  const text = [
    ...
    ``,
    `PLEASE REPLY WITH TWO THINGS`,
    ``,
    `1. ${REQUEST_NOW.title}.`,
    REQUEST_NOW.body,
    ``,
    `2. ${REQUEST_PHOTO.title}.`,
    REQUEST_PHOTO.body,
    ``,
    `WHEN WE ARE AROUND`,
  ...
  <tr><td style="padding:26px 30px 0;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
      ...
      <tr><td style="padding:20px 22px;">
        <div style="font-size:12.5px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#A8702B;">Two things we need from you</div>

        <div style="font-size:16.5px;font-weight:700;color:#222A35;margin-top:10px;">1. ${esc(REQUEST_NOW.title)}</div>
        <p style="margin:6px 0 0;font-size:14.4px;color:#525E6D;line-height:1.65;">${esc(REQUEST_NOW.body)}</p>

        <div style="font-size:16.5px;font-weight:700;color:#222A35;margin-top:16px;">2. ${esc(REQUEST_PHOTO.title)}</div>
        <p style="margin:6px 0 0;font-size:14.4px;color:#525E6D;line-height:1.65;">${esc(REQUEST_PHOTO.body)}</p>

        <p style="margin:16px 0 0;">
          <a href="mailto:${esc(supportEmail)}?subject=${encodeURIComponent('Certificate and photo — ' + reference)}"
             style="display:inline-block;background:#E0A35E;color:#222A35;text-decoration:none;font-weight:700;font-size:14.5px;padding:11px 20px;border-radius:999px;">Reply with both</a>
        </p>
      </td></tr>
    </table>
  </td></tr>
