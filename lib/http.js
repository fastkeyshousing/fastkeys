...
export async function rateLimit(db, key, limit, windowSeconds) {
  ...
  try {
    ...
    /* Failing open is deliberate: a broken limiter must not take the application
     * form offline. "no such table" almost always means migrations have not been
     * applied to whichever database this environment is pointing at. */
    const hint = /no such table/i.test(String(err))
      ? ' Run `npm run db:migrate:local` (or `npm run db:migrate` for production).'
      : '';
    console.error(`[rate-limit] backend error, failing open:${hint}`, err);
    return { ok: true };
  }
}
