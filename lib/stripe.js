...
export function createCheckoutSession(env, { application, reference, siteUrl, idempotencySuffix }) {
  ...
  /* Not /apply: someone whose card was declined has already filled the form, and
   * sending them back to a blank one is how you lose them. The failure page can
   * explain the decline and offer a retry against the stored application. */
  form.set('cancel_url', `${siteUrl}/payment-failed?session_id={CHECKOUT_SESSION_ID}`);
  ...
  return call(env, '/checkout/sessions', {
    method: 'POST',
    body: form,
    /* Keyed on the application so a double-submitted form cannot produce two
     * charges. A retry passes a suffix, because reusing the key would hand back
     * the original, already-dead session. */
    idempotencyKey: `checkout-${application.id}${idempotencySuffix ? `-${idempotencySuffix}` : ''}`,
  });
}

/* Checkout for a viewing. Distinct from the application checkout in three ways
 * that matter downstream: a different price, a different pair of return URLs,
 * and metadata.kind, which is how the webhook tells the two apart when Stripe
 * calls back. Without that marker every handler would have to guess. */
export function createViewingCheckout(env, { viewing, id, reference, priceId, siteUrl }) {
  const form = new URLSearchParams();
  form.set('mode', 'payment');
  form.set('line_items[0][price]', priceId);
  form.set('line_items[0][quantity]', '1');
  form.set('success_url', `${siteUrl}/viewing-confirmed?session_id={CHECKOUT_SESSION_ID}`);
  form.set('cancel_url', `${siteUrl}/viewing-failed?session_id={CHECKOUT_SESSION_ID}`);
  form.set('client_reference_id', reference);
  form.set('customer_email', viewing.email);
  form.set('metadata[kind]', 'viewing');
  form.set('metadata[viewing_id]', id);
  form.set('metadata[reference]', reference);
  form.set('metadata[service]', viewing.service);
  /* Truncated: Stripe caps a metadata value at 500 characters, and a long
   * listing URL would otherwise fail the whole call. */
  form.set('metadata[property]', String(viewing.property_url).slice(0, 480));
  form.set('payment_intent_data[metadata][kind]', 'viewing');
  form.set('payment_intent_data[metadata][reference]', reference);
  form.set('payment_intent_data[description]',
    `FastKeys ${viewing.service} viewing ${reference}`);
  form.set('locale', 'auto');

  return call(env, '/checkout/sessions', {
    method: 'POST',
    body: form,
    idempotencyKey: `viewing-${id}`,
  });
}
