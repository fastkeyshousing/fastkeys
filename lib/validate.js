...
export function validateApplication(raw) {
  ...
  /* Optional, because thousands of applications already exist without it and a
   * required field would lock every one of them out of an edit. When it is
   * given it has to be a real date and a plausible age: a tenancy needs an
   * adult, and a typo in the year is the commonest way this field goes wrong. */
  const dateOfBirth = text(raw.date_of_birth, 10);
  if (dateOfBirth) {
    need(DATE_RE.test(dateOfBirth), 'date_of_birth');
    const born = new Date(`${dateOfBirth}T00:00:00Z`);
    need(!Number.isNaN(born.getTime()), 'date_of_birth');
    const years = (Date.now() - born.getTime()) / (365.2425 * 24 * 3600 * 1000);
    need(years >= 18, 'date_of_birth_under_18');
    need(years <= 100, 'date_of_birth_implausible');
  }

  const availableFrom = text(raw.available_from ?? raw.from, 10);
  need(DATE_RE.test(availableFrom), 'available_from');

  /* The browser bounds the picker, but min/max are a convenience for the person
   * filling the form, not a control: anything can be posted here directly. A
   * day of slack absorbs the gap between the applicant's clock and UTC, so
   * somebody picking "today" from a timezone behind us is not rejected. */
  if (DATE_RE.test(availableFrom)) {
    const day = 86_400_000;
    const floor = new Date(Date.now() - day).toISOString().slice(0, 10);
    const ceiling = new Date(Date.now() + 730 * day).toISOString().slice(0, 10);
    const normalised = availableFrom.length === 7 ? `${availableFrom}-01` : availableFrom;
    need(normalised >= floor, 'available_from_past');
    need(normalised <= ceiling, 'available_from_far');
  }
