...
export async function sendAndLog(env, {
  reference, kind, to, subject, html, text, sentBy, meta, idempotencyKey,
}) {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers,
      body: JSON.stringify({ from: env.NOTIFY_FROM, to: [to], reply_to: support, subject, html, text }),
    });
  ...
  try {
    await env.DB.prepare(
      ...
      crypto.randomUUID(), reference, kind, subject.slice(0, 200), to,
