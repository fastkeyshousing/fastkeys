...
/* Tables, not flexbox: Outlook ignores flex, so a multi-row thumbnail grid has
 * to be built from real table rows. */
...
export function propertyEmail({
  ...
  /* Twelve rather than the thirty a listing may hold. Gmail clips a message past
   * roughly 102 KB and hides the rest behind "View entire message", which is
   * where the reply instruction would end up. The link to the full listing
   * carries the remainder. */
  const safeImages = images.map(safeUrl).filter(Boolean).slice(0, 12);
  ...
  const text = [
    ...
    ``,
    `You have already signed up and paid, so there is nothing to fill in again`,
    `and no button to press. Do not use the apply link on our website: that is`,
    `for new clients and it would only start a second application.`,
    ``,
    `Reply here and tell us one of two things:`,
    `  YES  - we put your application in front of the landlord today.`,
    `  NO   - tell us what is wrong with it, and the next one gets closer.`,
    ``,
    `Either answer is useful. Places in Maastricht move within days, so a quick`,
    `reply beats a considered one.`,
    ``,
    `FastKeys`,
    siteUrl,
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff;border-radius:16px;overflow:hidden;">
  ...
  ${safeImages.length ? `<tr><td style="padding:${intro ? '8' : '24'}px 30px 0;">
    ...
    ${safeImages.length > 1 ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:8px;">
      ${chunk(safeImages.slice(1), 3).map((rowOf) => `<tr>
        ${rowOf.map((src) => `<td width="33%" style="padding:0 4px 8px 0;">
          <img src="${esc(src)}" alt="" style="width:100%;height:auto;border-radius:9px;display:block;">
        </td>`).join('')}
        ${Array.from({ length: 3 - rowOf.length }, () => '<td width="33%"></td>').join('')}
      </tr>`).join('')}
    </table>` : ''}
  </td></tr>` : ''}
  ...
  <tr><td style="padding:24px 30px 0;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#FFF7EC;border:1.5px solid rgba(224,163,94,.5);border-radius:12px;">
      <tr><td style="padding:20px 22px;">
        <div style="font-size:17px;font-weight:700;color:#222A35;">Just reply to this email</div>
        <p style="margin:8px 0 0;font-size:14.8px;line-height:1.68;color:#525E6D;">
          You have already signed up and paid, so there is nothing to fill in again and no
          button to press. <strong style="color:#222A35;">Please do not use the apply link on
          our website</strong> &mdash; that is for new clients and it would only start a second
          application.
        </p>
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:14px;">
          <tr><td style="padding:9px 0;border-top:1px solid rgba(224,163,94,.35);">
            <strong style="color:#222A35;font-size:15px;">Yes, I want it</strong>
            <span style="color:#525E6D;font-size:14.4px;"> &mdash; we put your application in front of the landlord today.</span>
          </td></tr>
          <tr><td style="padding:9px 0;border-top:1px solid rgba(224,163,94,.35);">
            <strong style="color:#222A35;font-size:15px;">No, not this one</strong>
            <span style="color:#525E6D;font-size:14.4px;"> &mdash; tell us what is wrong with it and the next one gets closer.</span>
          </td></tr>
        </table>
        <p style="margin:14px 0 0;font-size:14.4px;line-height:1.65;color:#525E6D;">
          Either answer is useful. Places here move within days, so a quick reply beats a
          considered one.
        </p>
        <p style="margin:14px 0 0;">
          <a href="mailto:${esc(supportEmail)}?subject=${encodeURIComponent(`Re: ${headline || address || 'the property you sent'}${reference ? ` (${reference})` : ''}`)}"
             style="display:inline-block;background:#E0A35E;color:#2A2116;text-decoration:none;font-weight:700;font-size:15px;padding:12px 22px;border-radius:999px;">Reply about this property</a>
        </p>
      </td></tr>
    </table>
  </td></tr>
