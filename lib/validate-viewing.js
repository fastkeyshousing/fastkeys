...
export function validateViewing(raw) {
  ...
  /* Now required. A viewing where nobody told us what to look for is a viewing
   * that comes back with a summary the buyer did not need. */
  const questions = text(raw.questions, 1200);
  need(questions.length >= 10, 'questions');
