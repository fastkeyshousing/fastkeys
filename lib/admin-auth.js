 ...
 * OWASP suggests more, but this is a platform ceiling, not a preference.
 *
 * What carries the weight instead: a 16-byte random salt per account, a
 * twelve-character minimum, and eight attempts per account per fifteen minutes.
 * Offline cracking still requires somebody to have dumped the database first, at
 * which point the applicant data in it is the larger problem. */
...
export async function hashPassword(password, saltHex, iterations = ITERATIONS) {
  /* An account created before this ceiling was known carries an unusable count.
   * Throwing is right: silently clamping would compute a different hash and then
   * report it as a wrong password, which is the failure that wastes an evening. */
  if (iterations > MAX_ITERATIONS) {
    const err = new Error(`stored iteration count ${iterations} exceeds the ${MAX_ITERATIONS} this runtime allows`);
    err.code = 'ITERATIONS_TOO_HIGH';
    throw err;
  }
...
export { ITERATIONS, MAX_ITERATIONS, SESSION_HOURS };
