...
export function reminderEmail({ application, reference, resumeUrl, siteUrl, supportEmail }) {
  ...
  const text = [
    `Hi ${first},`,
    ``,
    `You started an application with us for ${application.city || 'a place'} but the`,
    `payment did not go through, so we have not started searching yet.`,
    ``,
    ...
    ``,
    `If you have changed your mind, that is completely fine and you can ignore`,
    `this. We will not chase you again.`,
    ``,
...
<title>Your application is still waiting</title></head>
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fff;border-radius:16px;overflow:hidden;">
  ...
  <tr><td style="background:#445162;padding:24px 30px;">
    ...
    <div style="color:#fff;font-size:21px;font-weight:600;margin-top:8px;line-height:1.3;">Your application is still waiting, ${esc(first)}.</div>
  </td></tr>

  <tr><td style="padding:26px 30px 0;">
    <p style="margin:0;font-size:15.5px;line-height:1.65;color:#525E6D;">
      You started an application for ${city}, but the payment did not go through, so we
      have not started searching yet. <strong style="color:#222A35;">Nothing was charged.</strong>
    </p>
    ...
    <p style="margin:22px 0 0;">
      <a href="${esc(resumeUrl)}" style="display:inline-block;background:#E0A35E;color:#222A35;text-decoration:none;font-weight:700;font-size:15.5px;padding:14px 26px;border-radius:999px;">Finish your application &rarr;</a>
    </p>
  </td></tr>
  ...
  <tr><td style="padding:22px 30px 28px;">
    <p style="margin:0;font-size:14.2px;line-height:1.65;color:#7A8593;">
      Changed your mind? That is completely fine, ignore this and we will not chase you again.
  ...
  return {
    subject: `You didn't finish your FastKeys application`,
    text,
    html,
  };
}
