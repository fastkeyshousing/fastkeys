...
export function telegramMessage(reference, amount, body) {
  const ok = amount !== 'payment failed';
  const header = ok
    ? `✅ Paid application ${reference} (${amount})\n\n`
    : `⚠️ Payment FAILED for ${reference}\n\n`;
...
/* A payment that bounced. Short by design: no money moved, no work is owed, and
 * the full application is already in D1 if it turns out to matter. */
export async function notifyPaymentFailed(env, { reference, name, email, reason }) {
  const body =
    `Reference: ${reference}\n` +
    `Name:      ${name}\n` +
    `Email:     ${email}\n` +
    `Reason:    ${reason || 'not given by the bank'}\n\n` +
    `No charge was taken. The application is stored but unpaid, and the applicant ` +
    `has been told on the confirmation page that they can try again.`;

  const settled = await Promise.allSettled([
    sendTelegram(env, reference, 'payment failed', body),
    sendEmail(env, reference, 'payment failed', body),
  ]);
  const outcomes = settled.map((r) =>
    r.status === 'fulfilled' ? `${r.value.channel}=${r.value.status}` : `error: ${r.reason?.message || r.reason}`
  );
  for (const r of settled) {
    if (r.status === 'rejected') console.error('[notify:failed]', r.reason);
  }

  const sent = settled.some((r) => r.status === 'fulfilled' && r.value.status === 'sent');
  /* Logged either way, matching notifyPaid. A notification path that is silent
   * when it works is a path you cannot tell apart from one that never ran. */
  console[sent ? 'log' : 'error'](
    `[notify] payment failure for ${reference} ${sent ? 'reported' : 'NOT reported'} (${outcomes.join(', ')})`
  );
  return sent;
}
