...
export function parseText(raw) {
  ...
  /* Ordered most specific first. A labelled price beats a bare number, and a
   * number next to "per month" beats one that just happens to be in the text,
   * because posts are full of numbers: postcodes, sizes, house numbers, dates. */
  const rentMatch = first(text, [
    /(?:kale\s+)?(?:huur(?:prijs)?|rent(?:al)?(?:\s+price)?|prijs|price|asking|vraagprijs)\s*(?:per\s*maand|p\/m|pm)?\s*[:=\-–]?\s*(?:€|eur|euro)?\s*([\d][\d.,]*)/i,
    /(?:€|eur\b|euro\b)\s*([\d][\d.,]*)\s*(?:,-|,00|,--)?\s*(?:p\/m|p\.m\.|per\s*maand|per\s*month|pm\b|\/\s*month|\/\s*maand|month|maand)/i,
    /([\d][\d.,]*)\s*(?:euro|eur|€)\s*(?:,-)?\s*(?:p\/m|per\s*maand|per\s*month|pm\b|\/\s*month|a\s*month)/i,
    /(?:€|eur\b|euro\b)\s*([\d][\d.,]*)/i,
    /([\d][\d.,]*)\s*(?:euro|eur)\b/i,
    /([\d][\d.,]*)\s*(?:,-)\s*(?:p\/m|per\s*maand)?/i,
  ]);
  let rent = money(rentMatch && rentMatch[1]);
  /* A postcode, a house number or a year is not a rent. Anything outside a
   * plausible monthly rent is discarded rather than published. */
  if (rent !== null && (rent < 150 || rent > 6000)) rent = null;
  if (rent) {
...
export function parseHtml(html, pageUrl) {
  /* A site-specific reader first where one exists. Generic extraction on a
   * Next.js app returns the site's own title and description, which is worse
   * than nothing because it looks like it worked. */
  const rentHunter = parseRentHunter(html);
  if (rentHunter) return rentHunter;
...
/* ------------------------------------------------------- RentHunter -------
 *
 * RentHunter is a Next.js app. Its pages carry no useful og: tags or JSON-LD:
 * those describe the site, not the listing, which is exactly why a generic
 * reader filled the boxes with the site's own title and description.
 *
 * The listing itself is streamed in the React flight payload as a series of
 * self.__next_f.push([1,"<chunk>"]) calls, and inside it sits a complete
 * advertFromServer object with the real fields. Reading that is not a guess:
...
function flightPayload(html) {
  ...
  while ((m = re.exec(html)) !== null) {
    try { chunks.push(JSON.parse(m[1])); } catch { /* a chunk we cannot read is skipped */ }
  }
  return chunks.join('');
}

/* Walks the braces to find the end of the object rather than trying to match it
 * with a regular expression, which cannot handle nesting. */
...
/* RentHunter stores originals on a Scaleway bucket and serves them through an
 * imgproxy instance at img1.renthunter.nl. The proxied URL is built in the
 * browser and never appears in the HTML, so it has to be constructed.
 *
 * Worth doing rather than taking the bucket URL directly: the proxy returns a
 * resized webp instead of a full-size original, which is a fraction of the
 * bytes for the same visible quality, and it means we are pulling from the
 * endpoint they expose rather than straight from their storage.
 *
 * The plain original is kept alongside so a failed proxy fetch can fall back to
 * it rather than losing the photo. */
const RH_PROXY = 'https://img1.renthunter.nl/insecure/size:3840:0/resizing_type:fill/quality:85/plain/';

export function rentHunterImage(url) {
  const raw = String(url || '');
  if (!/^https?:\/\//.test(raw)) return null;
  /* Already proxied: leave it alone. */
  if (raw.startsWith('https://img1.renthunter.nl/')) return raw;
  if (!/rentsaver\.s3\.[a-z0-9-]+\.scw\.cloud\//i.test(raw)) return raw;
  return `${RH_PROXY}${raw}@webp`;
}

/* The reverse, for the fallback: pull the original back out of a proxied URL. */
export function unproxyImage(url) {
  const m = String(url || '').match(/\/plain\/(https?:\/\/.+?)(?:@[a-z]+)?$/i);
  return m ? m[1] : null;
}
...
function niceDate(raw) {
  ...
  if (Number.isNaN(d.getTime())) return String(raw).slice(0, 40);
  return `${d.getUTCDate()} ${MONTHS[d.getUTCMonth()]} ${d.getUTCFullYear()}`;
}

export function parseRentHunter(html) {
  const advert = objectAfter(flightPayload(html), '"advertFromServer":');
  if (!advert || typeof advert !== 'object') return null;

  const loc = advert.location_details_from_api || {};
  const det = advert.location_details || {};
  const out = {};

  /* The API address is already formatted and corrected; the scraped one is the
   * fallback because it is what the source site said, typos and all. */
  const street = loc.route || det.street;
  const number = loc.house_number || [det.house_number, det.unit].filter(Boolean).join('');
  const address = loc.address
    || [street && number ? `${street} ${number}` : street, loc.postal_code].filter(Boolean).join(', ');
  if (address) out.address = String(address).slice(0, 240);
  if (loc.city || advert.city) out.city = String(loc.city || advert.city).replace(/\b\w/g, (c) => c.toUpperCase());

  /* price is the advertised total. base_rent plus other_costs is what a tenant
   * actually pays, so where the breakdown exists it is worth saying so. */
  /* Two decimals or none. toLocaleString alone renders 439.5 as "439,5",
   * which reads like a typo on a price. */
  const euros = (n) => {
    const num = Number(n);
  ...
  const bd = advert.price_breakdown || {};
  if (advert.price) {
    const inclusive = bd.base_rent && bd.other_costs && (bd.base_rent + bd.other_costs) <= advert.price;
    out.rent = `${euros(advert.price)}${inclusive ? ' incl.' : ''}`;
  } else if (bd.base_rent) {
    out.rent = `${euros(bd.base_rent)} excl.`;
  }

  if (advert.deposit) out.deposit = euros(advert.deposit);
  else if (advert.deposit_months) out.deposit = `${advert.deposit_months} month(s) rent`;

  if (advert.surface) out.size = `${advert.surface} m²`;

  const type = advert.converted_type || advert.type;
  const beds = advert.bedrooms;
  if (beds) out.rooms = `${beds} bedroom${beds === 1 ? '' : 's'}`;
  else if (advert.rooms) out.rooms = `${advert.rooms} room${advert.rooms === 1 ? '' : 's'}`;
  if (type && !out.rooms) out.rooms = String(type).replace(/_/g, ' ').toLowerCase();

  const furn = advert.converted_furnishing || advert.furnishing;
  if (furn) {
    const f = String(furn).toUpperCase();
    out.furnished = f.includes('UNFURNISHED') || f.includes('SHELL') ? 'Unfurnished'
      : f.includes('UPHOLSTERED') ? 'Upholstered'
      : f.includes('FURNISHED') ? 'Furnished' : String(furn).toLowerCase();
  }

  /* The line students care about most, and RentHunter states it outright. */
  if (advert.registration_possible === true) out.registration = 'Yes';
  else if (advert.registration_possible === false) out.registration = 'No';

  const from = niceDate(advert.move_in_date);
  if (from) out.available_from = from;

  const desc = advert.description_en || advert.description || advert.description_nl;
  if (desc) out.description = String(desc).replace(/\s+/g, ' ').trim().slice(0, 3500);

  /* labelled_images carries an order and a role where it exists; images is the
   * plain list. Both are used, deduplicated, so nothing is missed. */
  const rawImages = [];
  for (const source of [advert.labelled_images, advert.images]) {
    if (!Array.isArray(source)) continue;
    for (const item of source) {
      const u = typeof item === 'string' ? item : (item?.url || item?.src || item?.image);
      if (typeof u === 'string' && /^https?:\/\//.test(u) && !rawImages.includes(u)) {
        rawImages.push(u);
      }
    }
  }
  if (rawImages.length) {
    out.images = rawImages.map(rentHunterImage).filter(Boolean).slice(0, 30);
  }

  /* Where RentHunter found it. Worth keeping: it is the landlord's own page and
   * the place to go for anything the payload does not carry. */
  if (advert.url) out.external_url = String(advert.url).slice(0, 500);
  if (advert.source) out.source_name = String(advert.source);

  out.title = [
    out.rooms ? out.rooms.replace(/\b\w/, (c) => c.toUpperCase()) : (type || 'Place'),
    street ? `on ${street}` : null,
  ].filter(Boolean).join(' ').slice(0, 160);

  const filled = ['address', 'rent', 'size', 'rooms'].filter((k) => out[k]).length;
  return filled >= 2 ? out : null;
}
